...
if (isset($_GET['judul'])){
   $sql = mysql_query("select judul from berita where judul_seo='$_GET[judul]'");
	...
	$sql = mysql_query("select jdl_video from video where id_video='$_GET[id]'");
    $j   = mysql_fetch_array($sql);
    if ($j) {
...
elseif ($_GET['ald'] != ''){
	$sql = mysql_query("select jdl_album from album where id_album='$_GET[ald]'");
    $j   = mysql_fetch_array($sql);
    if ($j) {
        echo "Album Foto - $j[jdl_album]";
}
}
elseif ($_GET['kt'] != ''){
	$sql = mysql_query("select nama_kategori from kategori where id_kategori='$_GET[kt]'");
    $j   = mysql_fetch_array($sql);
    if ($j) {
        echo "Informasi Kategori $j[nama_kategori]";
}
}
...
elseif (isset($_GET['haldownload'])){
	echo "Semua Daftar file Download Halaman $_GET[haldownload]";
}
elseif (isset($_GET['alb'])){
	echo "Semua Daftar Album Foto";
}
...
$sql = mysql_query("select tema from agenda where tema_seo='$_GET[tema]'");
    $j   = mysql_fetch_array($sql);
    if ($j) {
        echo "$j[tema]";
	}
...
elseif ($_GET['halaman'] != ''){
	$sql = mysql_query("SELECT * FROM halamanstatis where judul_seo='$_GET[halaman]'");
    $j   = mysql_fetch_array($sql);
    if ($j) {
